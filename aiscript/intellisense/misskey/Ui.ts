...
export default languages.registerCompletionItemProvider('aiscript', {
    provideCompletionItems(document, position) {
        ...
        if (!(/Ui:$/).test(linePrefix)) {
            ...
            const MkRoot = new CompletionItem('Ui:');
        ...
        return [
            new CompletionItem('root', CompletionItemKind.Constant),

            createCompletionItemKindMethod('patch'),
            createCompletionItemKindMethod('get'),
            createCompletionItemKindMethod('render'),
        ];
    }
}, ':');